if VoidUISBStandalone.fix then
	return
end

if VoidUISBStandalone.options.enable_stats then
	...
	if RequiredScript == "lib/managers/hud/newhudstatsscreen" then
		function HudTrackedAchievement:init(parent, id, i, h, scale)
			...
			self._panel:bitmap({
				texture = "guis/textures/VoidUISBStandalone/hud_weapons",
		...
		function HUDStatsScreen:init()
			self._scoreboard_enabled = VoidUISBStandalone.options.scoreboard
			self._scale = VoidUISBStandalone.options.scoreboard_scale
			self._timer = VoidUISBStandalone.options.enable_timer
			self._objective = VoidUISBStandalone.options.enable_objectives
		...
		function HUDStatsScreen:_create_stats_screen_profile(extras_panel)
			...
			if at_max_level and not VoidUISBStandalone.options.scoreboard_maxlevel then 
				return 
			end
		...
		function HUDStatsScreen:_update_stats_screen_loot(extras_panel, top_panel)
			...
			local accuracy = VoidUISBStandalone.options.scoreboard_accuracy and hit_accuracy and utf8.to_lower(managers.localization:text("menu_stats_hit_accuracy")):gsub("^%l", string.upper).." ".. hit_accuracy.."%" or ""
			local delay = VoidUISBStandalone.options.scoreboard_delay and trade_delay and " Ї "..managers.localization:text("hud_trade_delay", {TIME = tostring(self:_get_time_text(trade_delay))}) or ""
			...
			if self._scoreboard_panels and #self._scoreboard_panels > 0 and managers.achievment and #managers.achievment:get_tracked_fill() then
				local toggle_text = extras_panel:text({
					...
					font_size = 15 * self._scale,
					font = "fonts/font_medium_mf",
					text = (VoidUISBStandalone.options.scoreboard_toggle < 3 and self._toggle_text ~= "" and (utf8.to_upper(managers.localization:btn_macro(VoidUISBStandalone.options.scoreboard_toggle == 1 and "jump" or "duck")) .." ") or "")..self._toggle_text,
					vertical = "top",
					align = "center",
					layer = 1,
				})
				...
				local toggle_image = extras_panel:bitmap({
					...
					w = VoidUISBStandalone.options.scoreboard_toggle == 3 and toggle_text:h() / 1.5 or 0,
					h = toggle_text:h(),
					layer = 2
				})
		...
		function HUDStatsScreen:mouse_pressed(o, k)
			...
			if k == Idstring("1") and VoidUISBStandalone.options.scoreboard_toggle == 3 then
				self:toggle_panels()
			end
		end
		...
		function HUDStatsScreen:show()
			...
			self._key = managers.controller:get_settings(type):get_connection(VoidUISBStandalone.options.scoreboard_toggle == 1 and "jump" or "duck"):get_input_name_list()[1]
			...
			self._blur:set_visible(VoidUISBStandalone.options.scoreboard_blur)
			left_panel:stop()
		...
		function HUDStatsScreen:_animate_show_stats_left_panel(left_panel, full_hud_panel, top_panel, scoreboard_panel, extras_panel, mutators_panel)
			...
			-- 	timer_panel:set_layer(VoidUISBStandalone.options.show_timer == 2 and self._full_hud_panel:layer() + 1 or 0)
			-- 	timer_panel:set_visible(VoidUISBStandalone.options.show_timer > 1)
			-- end
			...
			-- 	obj_panel:set_layer(self._objective and (VoidUISBStandalone.options.show_objectives == 2 and self._full_hud_panel:layer() + 1 or 0) or 0)
			-- 	obj_panel:set_visible(self._objective and VoidUISBStandalone.options.show_objectives > 1 or true)
			-- 	if self._timer and VoidUISBStandalone.options.show_timer > 1 then
			-- 		obj_panel:set_y(self._objective and 0 or 40 * managers.hud._hud_heist_timer._scale)
			-- 	elseif (self._timer and not VoidUISBStandalone.options.show_timer > 1) or not self._timer then
			...
			while TOTAL_T > t do
				...
				self:align_scoreboard_panels()
				-- if self._timer and managers.hud._hud_heist_timer and managers.hud._hud_heist_timer._enabled then 
				-- 	timer_panel:set_alpha(math.lerp(timer_a, VoidUISBStandalone.options.show_timer > 1 and 1 or 0, t / TOTAL_T))
				-- end
				-- if self._objective and managers.hud._hud_objectives then 
				-- 	obj_panel:set_alpha(math.lerp(obj_a, VoidUISBStandalone.options.show_objectives > 1 and 1 or 0, t / TOTAL_T))
				-- end
			end
		...
		function HUDStatsScreen:_animate_hide_stats_left_panel(left_panel, full_hud_panel, top_panel, scoreboard_panel, extras_panel, mutators_panel)
			...
			-- 	obj_panel = managers.hud._hud_objectives._hud_panel:child("objectives_panel")
			-- 	obj_a = obj_panel:alpha()
			-- 	if (self._timer and VoidUISBStandalone.options.show_timer == 2 and VoidUISBStandalone.options.show_objectives == 3) or not self._timer then
			-- 		obj_panel:set_y(self._objective and -(32 * managers.hud._hud_objectives._scale) or 0)
			-- 	elseif self._timer and not VoidUISBStandalone.options.show_timer == 2 and not VoidUISBStandalone.options.show_objectives == 3 then
			-- 		obj_panel:set_y(self._objective and 0 or 40 * managers.hud._hud_heist_timer._scale)
			-- 	end
			-- end
			local t = 0
			while TOTAL_T > t do
				...
				self:align_scoreboard_panels()
			-- 	if self._timer and managers.hud._hud_heist_timer and managers.hud._hud_heist_timer._enabled then 
			-- 		timer_panel:set_alpha(math.lerp(timer_a, VoidUISBStandalone.options.show_timer == 3 and 1 or 0, t / TOTAL_T))
			-- 	end
			-- 	if self._objective and managers.hud._hud_objectives then 
			-- 		obj_panel:set_alpha(math.lerp(obj_a, VoidUISBStandalone.options.show_objectives == 3 and 1 or 0, t / TOTAL_T))
			-- 	end
			end
			-- if self._timer and managers.hud._hud_heist_timer and managers.hud._hud_heist_timer._enabled then 
			-- 	timer_panel:set_layer(0)
			-- 	timer_panel:set_visible(VoidUISBStandalone.options.show_timer == 3)
			-- end
			-- if self._objective and managers.hud._hud_objectives then 
			-- 	obj_panel:set_layer(0)
			-- 	obj_panel:set_visible(VoidUISBStandalone.options.show_objectives == 3)
			-- end
		...
		function HUDStatsScreen:update(t, dt)
			...
			if self._showing_stats_screen == true and VoidUISBStandalone.options.scoreboard_toggle < 3 and self._input_keyboard:pressed(Idstring(self._key)) then
				self:toggle_panels()
			end
		end
		...
		function HUDStatsScreen:create_scoreboards()
			...
			if self._scoreboard_enabled then
				...
				local labels = {
					{parent = "name_bg", name = "name", text = (VoidUISBStandalone.options.scoreboard_character and (managers.localization:text("menu_preferred_character").. " / ") or "" ) ..managers.localization:text("VoidUI_player").. " / " .. (VoidUISBStandalone.options.scoreboard_skills and managers.localization:text("menu_st_skilltree") or ""), align = "left"},
			...
			if has_stage_data then
				...
				if level_data then
					...
					for i = 1, max_waves do
						waves_panel:bitmap({
							name = "panel_"..i-1,
							texture = "guis/textures/VoidUISBStandalone/hud_extras",
	...
	elseif RequiredScript == "lib/managers/hudmanagerpd2" then
		...
		function HUDScoreboard:init(i, scoreboard_panel, is_player, h, scale)
			...
			local character_icon = self._panel:bitmap({
				...
				w = VoidUISBStandalone.options.scoreboard_character and h or 0,
				h = h,
				layer = 2
			})
			local name = self._panel:text({
				...
				vertical = "center",
				font_size = 20 * self._scale,
				x = VoidUISBStandalone.options.scoreboard_character and character_icon:right() or 5 * self._scale,
				y = 2,
				w = 230 * self._scale,
				h = self._h / 2,
				font = "fonts/font_medium_mf",
			...
			local kills_bg = self._panel:bitmap({
				...
				texture = "guis/textures/VoidUISBStandalone/hud_weapons",
				texture_rect = {69,0,416,150},
				x = VoidUISBStandalone.options.scoreboard_kills > 1 and name_bg:right() + 5 * self._scale or name_bg:right(),
				w = VoidUISBStandalone.options.scoreboard_kills > 1 and h or 0,
				h = h,
				layer = 1
			})
			self._panel:text({
			...
			local specials_bg = self._panel:bitmap({
				...
				texture = "guis/textures/VoidUISBStandalone/hud_weapons",
				texture_rect = {69,0,416,150},
				x = VoidUISBStandalone.options.scoreboard_specials and kills_bg:right() + 5 * self._scale or kills_bg:right(),
				w = VoidUISBStandalone.options.scoreboard_specials and h or 0,
				h = h,
				layer = 1
			})
			self._panel:text({
			...
			local civs_bg = self._panel:bitmap({
				...
				texture = "guis/textures/VoidUISBStandalone/hud_weapons",
				texture_rect = {69,0,416,150},
				x = VoidUISBStandalone.options.scoreboard_civs and specials_bg:right() + 5 * self._scale or specials_bg:right(),
				w = VoidUISBStandalone.options.scoreboard_civs and h or 0,
				h = h,
				layer = 1
			})
			self._panel:text({
			...
			local downs_bg = self._panel:bitmap({
				...
				texture = "guis/textures/VoidUISBStandalone/hud_weapons",
				texture_rect = {69,0,416,150},
				x = VoidUISBStandalone.options.scoreboard_downs and civs_bg:right() + 5 * self._scale or civs_bg:right(),
				w = VoidUISBStandalone.options.scoreboard_downs and h or 0,
				h = h,
				layer = 1
			})
			self._panel:text({
			...
			local primary_bg = self._panel:bitmap({
				...
				texture = "guis/textures/VoidUISBStandalone/hud_weapons",
				texture_rect = {69,0,416,150},
				x = VoidUISBStandalone.options.scoreboard_weapons and downs_bg:right() + 5 * self._scale or downs_bg:right(),
				w = VoidUISBStandalone.options.scoreboard_weapons and h * 2 or 0,
				h = h,
				layer = 1
			})
			local primary_icon = self._panel:bitmap({
				...
				w = h * 1.8,
				h = VoidUISBStandalone.options.scoreboard_weapons and h * 0.8 or 0,
				layer = 3
			})
			...
			local primary_silencer = self._panel:bitmap({
				...
				visible = false,
				w = VoidUISBStandalone.options.scoreboard_weapons and h * 0.3 or 0,
				h = h * 0.3,
				layer = 4
			})
			...
			local secondary_bg = self._panel:bitmap({
				...
				texture = "guis/textures/VoidUISBStandalone/hud_weapons",
				texture_rect = {69,0,416,150},
				x = VoidUISBStandalone.options.scoreboard_weapons and primary_bg:right() + 5 * self._scale or primary_bg:x(),
				w = VoidUISBStandalone.options.scoreboard_weapons and h * 2 or 0,
				h = h,
				layer = 1
			})
			local secondary_icon = self._panel:bitmap({
				...
				w = VoidUISBStandalone.options.scoreboard_weapons and h * 1.8 or 0,
				h = h * 0.8,
				layer = 3
			})
			...
			local melee_bg = self._panel:bitmap({
				...
				texture = "guis/textures/VoidUISBStandalone/hud_weapons",
				texture_rect = {69,0,416,150},
				x = VoidUISBStandalone.options.scoreboard_weapons and secondary_bg:right() + 5 * self._scale or secondary_bg:x(),
				w = VoidUISBStandalone.options.scoreboard_weapons and h * 2 or 0,
				h = h,
				layer = 1
			})
			...
			local armor_bg = self._panel:bitmap({
				...
				texture = "guis/textures/VoidUISBStandalone/hud_weapons",
				texture_rect = {69,0,416,150},
				x = VoidUISBStandalone.options.scoreboard_armor and melee_bg:right() + 5 * self._scale or melee_bg:right(),
				w = VoidUISBStandalone.options.scoreboard_armor and h or 0,
				h = h,
				layer = 1
			})
			...
			local perk_bg = self._panel:bitmap({
				...
				texture = "guis/textures/VoidUISBStandalone/hud_weapons",
				texture_rect = {69,0,416,150},
				x = VoidUISBStandalone.options.scoreboard_perk and armor_bg:right() + 5 * self._scale or armor_bg:right(),
				w = VoidUISBStandalone.options.scoreboard_perk and h or 0,
				h = h,
				layer = 1
			})
			local perk_icon = self._panel:bitmap({
				...
				w = VoidUISBStandalone.options.scoreboard_perk and h * 0.8 or 0,
				h = h * 0.8,
				layer = 2,
				alpha = 0.6
			})
			...
			self._panel:text({
				...
				font_size = 15 * self._scale,
				x = perk_bg:x(),
				w = VoidUISBStandalone.options.scoreboard_perk and h / 1.1 or 0,
				h = h / 1.1,
			...
			local hours_bg = self._panel:bitmap({
				...
				texture = "guis/textures/VoidUISBStandalone/hud_weapons",
				texture_rect = {69,0,416,150},
				x = VoidUISBStandalone.options.scoreboard_playtime and perk_bg:right() + 5 * self._scale or perk_bg:right(),
				w = VoidUISBStandalone.options.scoreboard_playtime and h * 1.5 or 0,
				h = h,
				layer = 1
			})
			self._panel:text({
			...
			local skill_icon = self._panel:bitmap({
				...
				w = VoidUISBStandalone.options.scoreboard_playtime and h * 0.8 or 0,
				h = h * 0.8,
				layer = 2,
				alpha = 0.6,
				visible = false
			...
			local ping_bg = self._panel:bitmap({
				...
				texture = "guis/textures/VoidUISBStandalone/hud_weapons",
				texture_rect = {69,0,416,150},
				x = VoidUISBStandalone.options.scoreboard_ping and hours_bg:right() + 5 * self._scale or hours_bg:right(),
				w = VoidUISBStandalone.options.scoreboard_ping and h or 0,
				h = h,
				layer = 1
			})
			self._panel:text({
		...
		function HUDScoreboard:set_player(character_name, player_name, ai, peer_id)
			if self._name ~= player_name then
				...
				if ai or not VoidUISBStandalone.options.scoreboard_skills then name:set_h(self._h) name:set_y(0) else name:set_h(self._h / 2) name:set_y(2) end
				...
				skills_text:set_visible(VoidUISBStandalone.options.scoreboard_skills and not ai or false)
				perk_count:set_visible(not ai)
				hours:set_visible(not ai)
				skill_icon:set_visible(ai)
				secondary_icon:set_w(VoidUISBStandalone.options.scoreboard_weapons and (ai and self._h * 0.8 or self._h * 1.8) or 0)
				...
				elseif outfit then
					local texture, rarity = managers.blackmarket:get_weapon_icon_path(outfit.primary and outfit.primary.factory_id and managers.weapon_factory:get_weapon_id_by_factory_id(outfit.primary.factory_id) or "new_m4", VoidUISBStandalone.options.scoreboard_skins > 1 and outfit.primary and outfit.primary.cosmetics)
					primary_icon:set_image(texture)
					primary_rarity:set_visible(VoidUISBStandalone.options.scoreboard_skins == 2 and rarity and true or false)
					...
					texture, rarity = managers.blackmarket:get_weapon_icon_path(outfit.secondary and outfit.secondary.factory_id and managers.weapon_factory:get_weapon_id_by_factory_id(outfit.secondary.factory_id) or "glock_17", VoidUISBStandalone.options.scoreboard_skins > 1 and outfit.secondary and outfit.secondary.cosmetics)
					secondary_icon:set_image(texture)
					secondary_rarity:set_visible(VoidUISBStandalone.options.scoreboard_skins == 2 and rarity and true or false)
		...
		function HUDScoreboard:sync_bot_loadout(character_name)
			...
			if unit and alive(unit) then
				...
				if loadout then
					...
					local texture, rarity = managers.blackmarket:get_weapon_icon_path(primary or "new_m4", VoidUISBStandalone.options.scoreboard_skins > 1 and unit:inventory() and unit:inventory():equipped_unit():base() and {id = unit:inventory():equipped_unit():base()._cosmetics_id} or nil)
					primary_icon:set_image(texture)
					primary_rarity:set_visible(VoidUISBStandalone.options.scoreboard_skins == 2 and rarity and true or false)
					primary_rarity:set_image(rarity and rarity)
		...
		function HUDScoreboard:reset_downs_stat(stat)
			if not VoidUISBStandalone.options.scoreboard_downs_no_reset then
	...
	elseif RequiredScript == "lib/units/enemies/cop/copdamage" and VoidUISBStandalone.options.scoreboard and not VoidUISBStandalone.fix then
		VoidUISBStandalone.fix = true
