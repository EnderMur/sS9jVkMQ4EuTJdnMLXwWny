...
function VoidUIMenu:init()
	...
	if VoidUISBStandalone.menus and #VoidUISBStandalone.menus > 0 then
		for _, menu in pairs(VoidUISBStandalone.menus) do
			self:GetMenuFromJson(menu)
		end
		self:OpenMenu("options")
	end
	
	if VoidUISBStandalone.Warning == 1 then
			...
			if self._open_choice_dialog.panel:inside(x,y) then
				for i, item in pairs(self._open_choice_dialog.items) do
					if alive(item) and item:inside(x,y) and item:alpha() == 1 then
						...
						VoidUISBStandalone.options[parent_item.id] = i
						parent_item.value = i
...
function VoidUIMenu:SetMenuItemsEnabled(menu)
	for _, item in pairs(menu.items) do
		...
		if parents and type(parents) == "string" then
			enabled = VoidUISBStandalone.options[parents]
		elseif parents and type(parents) == "table" then
			for _, parent in pairs(parents) do
				if VoidUISBStandalone.options[parent] == false then
					enabled = VoidUISBStandalone.options[parent]
				end
			end
		end
...
function VoidUIMenu:SetItem(item, value, menu)
	...
	if item and type(item) == "table" and item.default_value ~= nil then
		if item.type == "toggle" then
			item.value = value
			VoidUISBStandalone.options[item.id] = value
		...
		elseif item.type == "color_select" then
			...
			item.value = value
			VoidUISBStandalone.options[item.id] = {value.red, value.green, value.blue}
		end
		self:CreateChangeWarning()
...
function VoidUIMenu:GetMenuFromJson(path)
	...
	if file then
		...
		for i, item in pairs(items) do
			...
			local enabled = true
			if VoidUISBStandalone.options and VoidUISBStandalone.options[item.id] ~= nil then
				value = VoidUISBStandalone.options[item.id]
			end
			
			if parents ~= nil and type(parents) == "string" and VoidUISBStandalone.options[parents] ~= nil then
				enabled = VoidUISBStandalone.options[parents]
			elseif parents ~= nil and type(parents) == "table" then
				for _, parent in pairs(parents) do
					if VoidUISBStandalone.options[parent] == false and VoidUISBStandalone.options[parent] ~= nil then
						enabled = VoidUISBStandalone.options[parent]
					end
				end
...
function VoidUIMenu:SetSlider(item, x, add)
	...
	if percentage then
		...
		item.value = value
		VoidUISBStandalone.options[item.id] = tonumber(value)
	end
end
...
function VoidUIMenu:CloseColorMenu(save)	
	...
	self._open_color_dialog.panel:animate(function(o)	
		...
		if save then
			local color = self._open_color_dialog.color
			VoidUISBStandalone.options[self._open_color_dialog.parent_item.id] = {color.red, color.green, color.blue}
			self._open_color_dialog.parent_item.value = color
			self:CreateChangeWarning()
		end
		local option_color = VoidUISBStandalone.options[self._open_color_dialog.parent_item.id]
			...
			for _, menu in pairs(self._menus) do
				for _, item in pairs(menu.items) do 
					if item.value ~= nil and item.default_value ~= nil then
						self:SetItem(item, VoidUISBStandalone.options[item.id], menu)
					end
				end
			end
			self._controller:enable()
